...
public class HanziToPingtin {
	...
	public static void main(String[] args) {
		...
		System.out.println("firstSpell=" + HanziToPingtin.getFirstSpell(str));

		String file_path = "F:/手机备份/Others/电子书";
		File file_d = new File(file_path);
		File[] files = file_d.listFiles();
		for (int i = 0; i < files.length; i++) {
			File file = files[i];
			String file_name = file.getName();
			file.renameTo(new File(file_path + File.separator + file_name
					+ ".txt"));

			// if(file_name.endsWith(".txt")){
			// String file_name_suf =
			// file_name.substring(0,file_name.lastIndexOf("."));
			// String new_path =
			// file_path+File.separator+HanziToPingtin.getFirstSpell(file_name_suf).replaceAll("[^\\d\\w]","")+"="+file_name;
