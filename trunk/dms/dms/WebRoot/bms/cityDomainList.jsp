...
<script>
	function deleteRule(id){
	 if (confirm("确实删除吗？操作不可恢复！")){
		location.href="cd.do?method=delete&id="+id;
	 }
	 return false;
	}
...
<div class="bmstop">
  <div class="bmstitle">站点城市管理 </div>
  <div class="bmsadd"><a href="cityDomain.jsp"><img src="<%=request.getContextPath()%>/images/gwbms_03.gif" width="16" height="16" border="0" align="absmiddle" />添加站点城市</a></div>
  <div class="clear"></div>
</div>
<div class="gwbmscon">
	
	<form name="cdForm" id="cdForm" method="post" action="cd.do?method=list">
  ...
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="bmstab" id="list-table">
    ...
    <tr>
      <td class="first-cell" ><mytld:domainTag domainId="${obj.siteId}"></mytld:domainTag></td>
      <td><mytld:cityTag cityId="${obj.city}"></mytld:cityTag></td>
      <td>${obj.url}</td>
      <td>${obj.sortOrder}</td>
      <td align="center">
      	<a href="cd.do?method=view&id=${obj.id}">编辑</a> | <a href="#" onclick="deleteRule(${obj.id});return false;">删除</a> </td>
      </tr>
	      </c:forEach>
    </c:if>
  </table>
  <div align="right"><mytld:turnPage name="page" action="cd.do?method=list" formName="cdListForm"></mytld:turnPage></div>
  </div>
</div>
</body>
</html>
