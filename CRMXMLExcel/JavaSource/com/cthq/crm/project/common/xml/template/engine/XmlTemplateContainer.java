...
public class XmlTemplateContainer {
	...
	/**
	 * 设置在没有XML模板文件下 产生XML数据文件
	 * 层次数据映射集合在方法结束之后将清空
	 * @param valueMap XML数据文件 映射中的KEY是XML中的节点名称，映射中KEY对应的数据则是XML节点的数据
	 * @param xmlLevelLoc XML层次节点的位置
	 * @param ownerParentLoc 设定当前XML层次节点的所属的父节点的位置
	 */
	public void setXMLNodeData(Map valueMap, int xmlLevelLoc, int ownerParentLoc) {
		//数据集合的数据类型判断标识
		int intXmlObjType = -1;
		//XML层次节点的一览
		if (xmlLevelList == null) {
		...
		xmlElement.setOwnParentLoc(ownerParentLoc);
		//设置节点的所属叶子节点数据
		setNodeOWnerLeafData(valueMap,xmlNodeColl, xmlElement);
	...
	/**
	 * 设置XML数据集合 XML层次节点中有属性
	 * 层次数据映射集合在方法结束之后将清空
	 * @param valueMap  XML层次节点中叶子节点数据集合
	 * @param eleAttrMap XML层次节点的属性数据集合
	 * @param xmlLevelLoc XML层次节点的位置
	 * @param ownerParentLoc 设定当前XML层次节点的所属的父节点的位置
	 */
	public void setXMLNodeData(Map valueMap, Map eleAttrMap, int xmlLevelLoc, int ownerParentLoc) {
		//数据集合的数据类型判断标识
		int intXmlObjType = -1;
		//XML层次节点的一览
		if (xmlLevelList == null) {
		...
		xmlElement.setOwnParentLoc(ownerParentLoc);
		//设置节点属性
		setElementNodeAttrData(eleAttrMap, xmlElement);
		//设置节点的所属叶子节点数据
		setNodeOWnerLeafData(valueMap,xmlNodeColl, xmlElement);
	...
	/**
	 * 设置XML数据集合 XML层次节点中有属性，叶子节点有属性
	 * 层次数据映射集合在方法结束之后将清空
	 * 
	 * @param valueMap  XML层次节点中叶子节点数据集合
	 * @param eleAttrMap XML层次节点的属性数据集合
	 * @param leafAttrMap 叶子节点的属性数据集合 映射中的KEY对应叶子节点的属性，映射中的数据是叶子节点的属性的映射集合。
	 * 						属性映射集合，有属性的名称和对应的数据组成
	 * @param xmlLevelLoc XML层次节点的位置
	 * @param ownerParentLoc 设定当前XML层次节点的所属的父节点的位置
	 */
	public void setXMLNodeData(Map valueMap, Map eleAttrMap, Map leafAttrMap , int xmlLevelLoc, int ownerParentLoc) {
		//数据集合的数据类型判断标识
		int intXmlObjType = -1;
		//XML层次节点的一览
		if (xmlLevelList == null) {
		...
		setElementNodeAttrData(eleAttrMap, xmlElement);
		//设置节点的所属叶子节点数据
		setNodeOWnerLeafData(valueMap,xmlNodeColl, xmlElement, leafAttrMap);
		//创建分析当前的层次节点中所属的父节点的位置情况和数据的情况
		createXMLOwnerParentNode(xmlLevelLoc, ownerParentLoc);
		list.add(xmlElement);
		context.put(xmlLevelPath,list);
		...
		//清除叶子节点的属性映射集合
		if (leafAttrMap != null) {
			Set set01 = leafAttrMap.keySet();
			Iterator iter01 = set01.iterator();
			for(;iter01.hasNext();) {
				Object obj = iter01.next();
				obj = null;
			}
			leafAttrMap.clear();
		}
	 ...
	 * @param xmlElement
	 */
	private void setElementNodeAttrData(Map attrMap, XMLElement xmlElement) {
		if (attrMap == null) {
			return;
		}
		if (attrMap.isEmpty()) {
		...
		for(;keyIter.hasNext();) {
			...
			xmlElement.addAttribute(attrName, attrValue);
		}	
	}
	/**
	 * 设定节点的数据和节点的属性数据
	 * @param xmlObjVal 叶子节点数据映射集合
	 * @param xmlNodeColl  XML节点元素集合
	 * @param xmlElement XML节点元素
	 * @param leafAttrMap 叶子节点的属性映射集合
	 */
	private void setNodeOWnerLeafData(Map xmlObjVal,
			XMLNodeCollection xmlNodeColl, 
			XMLElement xmlElement,
			Map leafAttrMap) {
			Set set = ((Map)xmlObjVal).keySet();
			Iterator keyIter = set.iterator();
			//根据XML模板数据节点组织实际的XML数据块的集合
			for(;keyIter.hasNext();) {
				...
				xmlNodeColl.AddNode();
				//设置叶子节点的
				xmlNodeColl.putNodeProperty(name, ((Map)xmlObjVal).get(name).toString());
				if (leafAttrMap == null) {
					continue;
				}
				if (leafAttrMap.containsKey(name)) {
					Map leafNodeMap = (Map)leafAttrMap.get(name);
					Set set01 = leafNodeMap.keySet();
					Iterator iter01 = set01.iterator();
					for(;iter01.hasNext();) {
						String attrName = (String)iter01.next();
						String attrValue = (String)leafAttrMap.get(attrName);
						xmlNodeColl.putAttribute(attrName, attrValue);
					}
				}
			}	
	}
	 ...
	 * @param xmlElement
	 */
	private void setNodeOWnerLeafData(final Object xmlObjVal,
			XMLNodeCollection xmlNodeColl, 
			XMLElement xmlElement ) {
		if (xmlObjVal instanceof Map) {
			Set set = ((Map)xmlObjVal).keySet();
