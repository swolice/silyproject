 ...
 * @author 蒋峰
 */
public class ExcelBookWrite extends ExcelSTLWrite{
	//EXCEL模板文档
	private Workbook gSTLBook = null;
	...
	public void init(String xlsTempFile, String xlsDescFile) throws Exception{
        try   {
            ...
            //  Excel获得文件 
            gSTLBook =  Workbook.getWorkbook(new File(xlsTempFile));
            //  打开一个文件的副本，并且指定数据写回到原文件 
        	gWriteBook  =  Workbook.createWorkbook( new File(xlsDescFile));
        } catch(Exception ex) { 
        	gSTLBook = null;
        	gWriteBook = null;
        	throw ex;
	...
	public Sheet getSTLWorkSheet(int intSheetIdx){
		return gSTLBook.getSheet(intSheetIdx);
	}
	/**
	 ...
	 * @return 单元格类的基本信息
	 */
	public synchronized ExcelCell getLocalCell(Sheet dtSheet, int col, int row) {
