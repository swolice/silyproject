...
public class ExpExcelOperator {
	 ...
	 */
	private WritableCellFormat getCellFormat(String flag){
...
//								copyCell.setLocStr("");
								...
								if(value.matches(".*%.+%.*")){
									...
									if (dataMap.get(key) == null) {
										...
										copyCell.setLocStr(value.replaceAll("%"+key+"%", dataMap.get(key).toString()));	
									}
								}
							}
	...
	public static void copyRows(ExcelSTLWrite excelSTLWr,int sheetLoc ,
			...
			int margin,Map dataMap, List list1) {
...
//										copyCell.setLocStr("");
										...
										String map_value = Matcher.quoteReplacement((String)dataMap.get(key));
//										String map_value =(String)dataMap.get(key);	
										copyCell.setLocStr(value.replaceAll("%"+key+"%", map_value));	
									}
								}
							}
			...
			for (int j = 0; j < ws.getColumns(); j++) {
				...
				if(content.matches(".*#.+#.*")){
					...
					if (con != null){
						...
						content.replaceAll("#"+key+"#", value), cf);
						try {
							ws.addCell(lab);
						} catch (RowsExceededException e) {
